...
static int ctl_open(struct config_mgr *cm, struct ctl *pctl)
{
    ...
    if (pctl->type != MIXER_CTL_TYPE_UNKNOWN) {
...
#if !defined WITH_TINYALSA_AOSP
    ctl = mixer_get_ctl(stream->cm->mixer, volctl->id);
#else
    ctl = mixer_get_ctl_by_name(stream->cm->mixer, volctl->name);
#endif
...
#else
    if (s->controls.volume_left.name != NULL) {
        if (s->controls.volume_right.name == NULL) {
#endif
...
#else
    s->controls.volume_left.name = NULL;
    s->controls.volume_right.name = NULL;
#endif
    return s;
}
...
#else
        ALOGE_IF(state->current.stream->controls.volume_left.name != NULL,
                                "Left volume control specified again");
#endif
...
#else
        ALOGE_IF(state->current.stream->controls.volume_right.name != NULL,
                                "Right volume control specified again");
#endif
...
#else
    streamctl->name = mixer_ctl_get_name(ctl);
    ALOGV("(%p) Added control '%s' name %s function '%s' range %u-%u",
                state->current.stream,
                name, streamctl->name, function, streamctl->min, streamctl->max);
#endif
...
static int open_config_file(struct parse_state *state, char *file)
{
    ...
    if (file == NULL) {
        property_get("ro.hardware", property, "generic");
